    ...
    case ''||'/' : // 模拟欢迎页,nodejs是高效流处理的方案,也可以通过配置文件来配置
        fs.readFile('./index.html', function(err, content){
            if(err) {
                ...
                //response.end();
				response.writeHead(200, { 'Content-Type':'text/html; charset="UTF-8"' });
				var str='<style type="text/css">body{background:#000;color:#fff;}div{margin:5px 0;}</style>';
				str+='<div><b style="color:green;"> 汤姆 </b>恭喜您，cdn static 服务已经开启</div>';
				str+='<div style="border:1px solid green;padding:10px;">';
				str+='<div>静态资源命名规范</div>';
				str+='<div>1.静态资源的文件夹不允许出现大写字母和空格</div>';
				str+='<div>2.静态资源的css和js，以及图片等文件的文件名要求全小写字母,不允许出现大写字母</div>';
				str+='<div>3.页面中对静态资源的引用地址必须和磁盘中文件名保持一致，不允许出现大写字母</div>';
				str+='</div>';
				str+='<div>'+err.message+'</div>';
				response.write(str);
                //response.write(err.message);
                response.end();
            } else {
