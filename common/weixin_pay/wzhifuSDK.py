...
class WxPayConf_pub(object):
    ...
    KEY = "qqqg007007qqqg007007qqqg007007qq"#API密钥

    #微信公众号身份的唯一标识。审核通过后，在微信发送的邮件中查看 用于app端
    APPID_APP = "wxcf5a92893add3554"
    APPSECRET_APP = "b455aeaac27ed1e5f159ea8f2a3681bf"
    MCHID_APP = ''

    #=======【异步通知url设置】===================================
    #异步通知url，商户根据实际开发过程设定
    NOTIFY_URL = "http://www.qqqg.com/orders/weixin/pay/payback"

    #=======【JSAPI路径设置】===================================
    #获取access_token过程中的跳转uri，通过跳转将code传入jsapi支付页面
    JS_API_CALL_URL = "http://m.zenmez.com/pay/?showwxpaytitle=1"

    #=======【证书路径设置】=====================================
    #证书路径,注意应该填写绝对路径

    SSLCERT_PATH = "common/weixin_pay/cert/apiclient_cert.pem"
    SSLKEY_PATH = "common/weixin_pay/cert/apiclient_key.pem"
...
class Common_util_pub(object):
    ...
    def getSignURLStr(self, obj):
        ...
        String = "{0}&key={1}".format(String,WxPayConf_pub.KEY)
        #签名步骤三：MD5加密
        sign_str = hashlib.md5(String).hexdigest()
        #签名步骤四：所有字符转为大写
        sign_str = sign_str.upper()
        return "{0}&sign={1}".format(String,sign_str)
...
class JsApi_pub(Common_util_pub):
    ...
    def getParameters_APP(self):
        """设置jsapi的app支付参数"""
        jsApiObj = {}
        jsApiObj["apiKey"] = WxPayConf_pub.APPID
        jsApiObj['orderId'] = self.prepay_id
        jsApiObj['mchId'] = WxPayConf_pub.MCHID
        timeStamp = int(time.time())
        jsApiObj['nonceStr'] = self.createNoncestr()
        jsApiObj["timeStamp"] = "{0}".format(timeStamp)
        jsApiObj["nonceStr"] = self.createNoncestr()
        jsApiObj["package"] = "SignWXPay"
        jsApiObj["signType"] = "MD5"
...
class Wxpay_client_pub(Common_util_pub):
    ...
    def createXml(self):
        ...
        self.parameters["appid"] = WxPayConf_pub.APPID  if self.parameters['trade_type'] != 'APP' else WxPayConf_pub.APPID_APP #公众账号ID  app端使用不同
        self.parameters["mch_id"] = WxPayConf_pub.MCHID  if self.parameters['trade_type'] != 'APP' else WxPayConf_pub.MCHID_APP   #商户号
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        ...
        self.parameters["spbill_create_ip"] =  self.ip #终端ip
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串
        self.parameters["sign"] = self.getSign(self.parameters)  #签名
        return  self.arrayToXml(self.parameters)
...
# def test():
#     c = HttpClient()
#     content = c.get("http://www.baidu.com")
#     print content
#     c2 = HttpClient()
#     assert id(c) == id(c2)
#
