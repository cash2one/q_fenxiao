...
ht.detail={
    ...
    bindEvent:function(){
        ...
        var $minus=$('.minus');
        var $plus=$('.plus');
        $tempBuyAmountObj.on('keyup',function(){
            var maxCount=parseInt(ht.detail.goodInfo.max_limit_quantity,10);
            var minCount=parseInt(ht.detail.goodInfo.min_limit_quantity,10);
            var canSave=parseInt(ht.detail.goodInfo.sale_quantity,10)-parseInt(ht.detail.goodInfo.warning_quantity,10);
            canSave=canSave>1?(canSave-1):canSave;
            var realMaxCount=maxCount<=canSave?maxCount:canSave;
            if($(this).val().length==1){
                var v=$(this).val().replace(/[^1-9]/g,'');
                v=parseInt(v,10);
                var val=(v==''||v<minCount)?minCount:v;
                val=v>realMaxCount?realMaxCount:val;
                $(this).val(val);
            }else{
            ...
            if($(this).val()>=realMaxCount){
                $plus.addClass('dis').children().addClass('u-icn14-2');
            }else{
                $plus.removeClass('dis').children().removeClass('u-icn14-2');
            }
        ...
        $plus.on('click',function(){
            ...
            var canSave=parseInt(ht.detail.goodInfo.sale_quantity,10)-parseInt(ht.detail.goodInfo.warning_quantity,10);
            canSave=canSave>1?(canSave-1):canSave;
            if(tempBuyAmount==canSave){
                $(this).addClass('dis');
                $(this).children().addClass('u-icn14-2');
                $overAmount.addClass('show').find('.desc').html('啊哦，已经达到最大购买数。');
                return;
            }
