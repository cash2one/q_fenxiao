;;(function(){
    ht.cart={
            ...
            $.ajax({
                ...
                success: function(r) {
                    ...
                    if(r.state==200){
                        ...
                        orderJson=_this.parseData(orderJson);
        ...
        getGoodObj:function($root){
            var oIndex=parseInt($root.attr('data-oIndex'),10);
            var pIndex=parseInt($root.attr('data-pIndex'),10);
            var goodsObj={
                orderO:this.oriData,
                goods:this.oriData[oIndex].items[pIndex],
                goodsO:this.oriData[oIndex].items,
                oIndex:oIndex,
                pIndex:pIndex
            }
            return goodsObj;
        ...
        getALLPrice:function($rootOrder){
            ...
            $rootOrder.find('.chosen').each(function(){
                ...
                if(goodObj.is_abroad=="True"){
                    if(tax_rate!=0){
                        tariffVal=tariffVal+parseFloat(parseInt(num,10))*parseFloat(goodObj.goods.price)*tax_rate/100;
                    }
                }else{
                    tariffVal=0;
                }
            ...
            var hourseObj=_this.getHouseObj($rootOrder).ordersObj;
            totalPrice=totalPrice.toFixed(2);
            tariffVal=tariffVal.toFixed(2);
            var tariff=0;
            if(parseFloat(tariffVal)<=ht.core.ops.tax[0].taxRate){
                tariff=0;
                $rootOrder.find('.tariffLbl').addClass('del');
            }else{
                tariff=parseFloat(tariffVal);
                $rootOrder.find('.tariffLbl').removeClass('del');
            }
            ...
            if(hourseObj.is_abroad=='True'){
                if(totalPrice>ht.core.ops.tax[0].maxPrice && finalChooseNum>1){
                    $rootOrder.find('.settlementBtn').addClass('u-btn-disab');
                    $rootOrder.find('.u-overtip').show();
                }else{
                    $rootOrder.find('.settlementBtn').removeClass('u-btn-disab');
                    $rootOrder.find('.u-overtip').hide();
                }
                if(finalChooseNum==0){
                    $rootOrder.find('.settlementBtn').addClass('u-btn-disab');
                    $rootOrder.find('.u-overtip').hide();
